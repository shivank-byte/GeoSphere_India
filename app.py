...
if st.session_state.rare_mode:
    ...
    hero_sub_text = RARE_LINES.get("GeoSphere India", "EARTH SCIENCE INTELLIGENCE PLATFORM") if st.session_state.get("rare_mode") else "EARTH SCIENCE INTELLIGENCE PLATFORM"
    st.markdown(f"""
    <div class='hero-banner'>
        <p class='hero-title'>GeoSphere India</p>
        <p class='hero-sub' style='{"font-style:italic;" if st.session_state.get("rare_mode") else ""}'>{hero_sub_text}</p>
        <span class='hero-badge'>EARTH SCIENCE · {datetime.datetime.now().strftime("%d %b %Y")}</span>
    </div>
    """, unsafe_allow_html=True)
