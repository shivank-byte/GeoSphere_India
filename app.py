...
<div class='glass-card' style='border-color:rgba(220,38,38,0.3);padding:14px 18px;margin-bottom:16px;'>
    <div style='color:#dc2626;font-size:0.62rem;letter-spacing:2px;margin-bottom:6px;'>📅 TODAY IN GEOLOGY — {_today.strftime("%d %B")}</div>
    <div style='color:#f0e6d0;font-size:0.85rem;line-height:1.7;font-family:Spectral,Georgia,serif;font-style:italic;'>{_event}</div>
</div>""", unsafe_allow_html=True)

# Live USGS earthquake count (last 24h, M2.5+ worldwide) — real-time metric
try:
    _r_eq = requests.get(
        ...
        + "&minmagnitude=2.5", timeout=4)
    _live_eq_count = _r_eq.json().get("count") if _r_eq.status_code == 200 else None
except Exception:
    _live_eq_count = None
