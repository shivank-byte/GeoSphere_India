...
for i, eq in enumerate(sorted(INDIA_EARTHQUAKES, key=lambda e: -e["year"])):
    with _hist_cols[i % 2]:
        ...
        st.markdown(f"""
        <div class='glass-card' style='padding:10px 14px;margin-bottom:6px;display:flex;justify-content:space-between;align-items:center;'>
            <div>
...
_pov = [
    ("🔍","Surface Rupture","Fresh scarps, offset roads/fences, ground cracks along fault trace — reveal which fault moved and by how much."),
    ("💧","Liquefaction","Sandy saturated soils behave like liquid — sand boils, foundation failures, lateral spreading near riverbanks."),
    ("🏔️","Landslides","Himalayan earthquakes routinely trigger thousands of slides — can dam rivers creating hazardous GLOF lakes."),
    ("🌊","Tsunami Potential","Any M7.5+ submarine quake near Andaman or Makran subduction zone requires immediate coastal hazard assessment."),
    ("📊","Aftershock Pattern","Spatial distribution maps the fault plane — reveals orientation, dimensions, which segments remain stressed."),
    ("🏠","Building Damage","Failure patterns reveal local site amplification — soft soils amplify shaking vs bedrock. Critical for planning."),
]
_pc = st.columns(2)
for i,(em,title,desc) in enumerate(_pov):
    with _pc[i%2]:
        st.markdown(f"""
