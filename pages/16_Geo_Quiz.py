...
if idx < total:
    ...
    if not st.session_state.quiz_answered:
        ...
        for oi, opt in enumerate(q["options"]):
            with opt_cols[oi % 2]:
                if st.button(f"{_letters[oi]}.  {opt}", key=f"qopt_{oi}"):
        ...
        for oi, opt in enumerate(q["options"]):
            if oi == correct:
                st.markdown(f"<div class='quiz-correct'>✅ {_letters[oi]}. {opt}</div>", unsafe_allow_html=True)
            elif oi == chosen:
                st.markdown(f"<div class='quiz-wrong'>❌ {_letters[oi]}. {opt}</div>", unsafe_allow_html=True)
            else:
                st.markdown(f"<div style='color:#334155;padding:2px 0;'>◻️ {_letters[oi]}. {opt}</div>", unsafe_allow_html=True)
        ...
        for oi, opt in enumerate(q["options"]):
            with rf_cols[oi%2]:
                if st.button(opt, key=f"rf_{idx}_{oi}", use_container_width=True):
                    ...
                    st.session_state.rf_start_time = time.time()
                    st.rerun()
        # Real live countdown: without this, the "15s" label only updates when a
        # button is clicked and just sits frozen otherwise. This forces one
        # rerun per second so the timer, and auto-advance at 0, actually work.
        time.sleep(1)
        st.rerun()
    else:
