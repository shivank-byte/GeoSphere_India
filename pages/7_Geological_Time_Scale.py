...
render_hero_image("Grand Canyon", "Geological Time Scale", "4.6 BILLION YEARS")
...
_default_idx = next((i for i, g in enumerate(GEOLOGIC_TIME) if g["period"] == "Cambrian"), 0)
sel_p = st.selectbox("Select a Period", _period_labels, index=_default_idx)
idx_p = _period_labels.index(sel_p)
...
_india_tl = [
    ("Hadean–Archean","4600–2500 Ma","Formation of Dharwar & Singhbhum cratons — India's ancient stable cores. Oldest Indian rocks >3 Ga (Karnataka, Jharkhand).","23:58:43","17 sec before midnight","Dharwar Craton"),
    ("Proterozoic","2500–541 Ma","Aravalli-Delhi orogeny. Vindhyan Supergroup deposition (sandstone, shale, limestone). India part of proto-Gondwana.","21:00","9 pm on the 24h scale","Vindhya Range"),
    ("Palaeozoic","541–252 Ma","India firmly in Gondwana. Gondwana Supergroup begins (coal swamps, Glossopteris flora across all southern continents).","22:30","10:30 pm","Glossopteris"),
    ("Mesozoic","252–66 Ma","Gondwana breaks up. India drifts north as an island. Jurassic marine sediments in Kutch. Rajasaurus roams the subcontinent.","23:30","11:30 pm","Gondwana"),
    ("K-Pg Boundary","66 Ma","Deccan Traps erupt (500,000 km² basalt). Combined with Chicxulub asteroid — mass extinction wipes out 75% of species.","23:49","11:49 pm","Deccan Traps"),
    ("Cenozoic","66–0 Ma","India collides with Eurasia ~50 Ma → Himalayas rise. Indo-Gangetic plain forms. Human ancestors appear in Siwalik fauna 2 Ma.","23:59:58","2 sec before midnight","Himalayas"),
]
for eon, trange, story, clock, clabel, photo_topic in _india_tl:
    _tl_img_col, _tl_txt_col = st.columns([1, 3])
    with _tl_img_col:
        show_wiki_photo(photo_topic, caption=eon)
    with _tl_txt_col:
        st.markdown(f"""
