...
geo_tab1, geo_tab2, geo_tab3 = st.tabs(["🌊 Rivers Map", "⛰️ Mountains Map", "🗺️ Combined + Layers"])
...
with geo_tab1:
    ...
    for i, r in enumerate(RIVERS):
        with rcols[i % 4]:
            show_wiki_photo(f"{r['name']} River", caption=r["name"])
            st.markdown(f"""
...
DESERT_REGIONS = [
    ...
    ("Kutch (Rann)", 23.7, 69.9, "Salt marsh / seasonal desert — the Great & Little Rann of Kutch. Tectonically active rift basin; site of the 2001 Bhuj earthquake."),
]
ISLAND_CHAINS = [
...
with geo_tab3:
    ...
    st.markdown("<div style='color:#c9a84c;font-size:0.85rem;margin:14px 0 6px;'>🏜️ Deserts & 🏝️ Island Chains</div>", unsafe_allow_html=True)
    di_cols = st.columns(5)
    for i, (name, lat, lon, desc) in enumerate(DESERT_REGIONS + ISLAND_CHAINS):
        with di_cols[i % 5]:
            show_wiki_photo(name, caption=name)
            st.markdown(f"""
            <div class='glass-card' style='padding:10px;'>
                <div style='color:#eab308;font-size:0.76rem;font-weight:600;margin-bottom:4px;'>{name}</div>
...
physio = [
    ("🏔️","Himalayan Mountains","2,500 km range. 3 parallel zones: Himadri (Greater Himalaya, >7000m), Himachal (Lesser Himalaya, 1000-4500m), and Siwaliks (outer foothills). Formed by ongoing India-Eurasia collision since ~50 Ma."),
    ("🌾","Northern Plains","700,000 km². Built from Ganga-Indus-Brahmaputra alluvium. Sub-divided into Bhabar, Terai, Bhangar and Khadar. India's most fertile, densely populated region."),
    ("⛰️","Peninsular Plateau","Oldest part of India — stable Precambrian craton >3 Ga. Average 600-900m elevation. Includes Deccan Plateau (basalt), Chota Nagpur (minerals), and Malwa Plateau."),
    ("🌊","Coastal Plains","Western (narrow, steep, Konkan + Malabar) and Eastern (broad, deltaic, Coromandel + Northern Circars)."),
    ("🏝️","Islands","Andaman & Nicobar (volcanic arc — India's only active volcano). Lakshadweep (coral atolls — geologically unrelated to Andamans)."),
]
pc = st.columns(5)
